    ...
    Memo.lazy_ (fun () ->
      ...
      then
        ...
        let+ local_packages =
          Dune_load.packages ()
          >>= fun pkgs ->
          (* Exclude vendored packages from lock dir validation *)
          Dune_lang.Package.Name.Map.to_list pkgs
          |> Memo.parallel_map ~f:(fun (name, pkg) ->
            let dir = Dune_lang.Package.dir pkg in
            Source_tree.is_vendored dir
            >>| fun vendored -> if vendored then None else Some (name, pkg))
          >>| List.filter_map ~f:Fun.id
          >>| Dune_lang.Package.Name.Map.of_list_exn
        ...
        | `Invalid ->
          ...
          in
          let runtime_deps =
            Dune_lang.Package.Name.Map.values local_packages
            |> List.concat_map ~f:(fun (pkg : Dune_pkg.Local_package.t) ->
              Dune_pkg.Dependency_formula.runtime_package_names pkg.dependencies
              |> Dune_pkg.Package_name.Set.to_list)
            |> Dune_pkg.Package_name.Set.of_list
          in
          (* Remove local packages from deps to get external deps only *)
          let current_deps =
            Dune_pkg.Package_name.Set.diff runtime_deps local_pkg_names
          in
          let locked_pkgs =
            Dune_pkg.Lock.Packages.to_pkg_list lock_dir.packages
...
let lock_context_rules ~dir components =
  ...
  | ctx_name :: [] ->
    ...
    (match Dev_tool.of_context_name ctx with
     | Some _dev_tool ->
       (* Dev tool lock dirs are generated by the solver directly, no rules needed here *)
       Memo.return @@ Gen_rules.rules_here Gen_rules.Rules.empty
     | None ->
       (* Regular context - set up project lock rules *)
       Lock_rules.setup_rules ~components:[ ".lock" ] ~dir)
  | _ :: _ ->
