...
let set_exclusion_paths t ~paths =
  ...
  State.critical_section t (fun t ->
    ...
    | Idle r | Start (r, _) ->
      (try Raw.set_exclusion_paths r paths with
       | Failure msg -> Code_error.raise msg [ "paths", Dyn.(list string) paths ]))
;;
