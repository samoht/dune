...
module Dir_map = struct
  module Per_dir = struct
    ...
    type t =
      ...
      ; files : Files.t
      ; vendor : Vendor_stanza.t Filename.Map.t
      }

    let to_dyn { sexps; subdir_status = _; files = _; vendor = _ } =
      let open Dyn in
      record
    ...
    let merge d1 d2 =
      ...
      ; vendor = Filename.Map.union d1.vendor d2.vendor ~f:(fun _ v _ -> Some v)
      }
    ;;
  end
...
module Ast = struct
  ...
  and decode ~inside_subdir ~inside_include =
    ...
    and+ vendor_stanzas = multi_field "vendor" vendor
...
module Group = struct
  ...
  let combine t (ast : Ast.t) =
    ...
    | Vendor v ->
      let vendor =
        match Filename.Map.add t.vendor v.directory v with
        | Ok map -> map
        | Error _ ->
          User_error.raise
            ~loc:v.loc
            [ Pp.textf "vendor stanza for directory %S already defined" v.directory ]
      in
      { t with vendor }
...
let rec to_dir_map ast =
  ...
  let node =
    ...
    let vendor = group.vendor in
    Dir_map.singleton
      { Dir_map.Per_dir.sexps = group.leftovers; subdir_status; files; vendor }
  in
  let subdirs =
...
let vendor t = (Dir_map.root t.plain).vendor
