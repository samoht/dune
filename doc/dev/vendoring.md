...
2. **Lock files compile to vendor infrastructure**
   ...
   - `dune pkg vendor` fetches sources to `duniverse/` and generates vendor stanzas
...
When running `dune pkg vendor`, dune generates `duniverse/dune` with vendor stanzas
for each vendored package:

```lisp
; duniverse/dune (auto-generated by dune pkg vendor)
(vendored_dirs *)
...
The `duniverse/dune` file is **fully regenerated** on each `dune pkg vendor`.
...
On `dune pkg vendor`, create `duniverse/.gitignore`:
...
When `dune pkg vendor` fetches packages to duniverse, it also writes opam files
...
1. `dune pkg vendor` derives package metadata from the opam repository
